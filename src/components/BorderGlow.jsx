...
const BorderGlow = ({
  ...
  useEffect(() => {
    if (!animated) return;
    const timingScale = animationDuration / 4000;
    const angleStart = 110;
    const angleEnd = 465;
    setSweepActive(true);
    setCursorAngle(angleStart);

    animateValue({ duration: 500 * timingScale, onUpdate: v => setEdgeProximity(v / 100) });
    animateValue({ ease: easeInCubic, duration: 1500 * timingScale, end: 50, onUpdate: v => {
      setCursorAngle((angleEnd - angleStart) * (v / 100) + angleStart);
    }});
    animateValue({ ease: easeOutCubic, delay: 1500 * timingScale, duration: 2250 * timingScale, start: 50, end: 100, onUpdate: v => {
      setCursorAngle((angleEnd - angleStart) * (v / 100) + angleStart);
    }});
    animateValue({ ease: easeInCubic, delay: 2500 * timingScale, duration: 1500 * timingScale, start: 100, end: 0,
  ...
  return (
    <div
      ...
      style={{
        ...
        borderColor: borderColor || (lightSurface ? 'rgb(24 24 27 / 12%)' : 'rgb(255 255 255 / 15%)'),
